...
-- ============================================================================
-- 15. Draft Prospects Table (Phase 1 - Draft System)
-- ============================================================================
CREATE TABLE draft_prospects (
    ...
    -- Physical Attributes
    height_inches INTEGER NOT NULL, -- Total inches (e.g., 6'2" = 74)
    weight INTEGER NOT NULL,
    age INTEGER NOT NULL,

    -- Ratings (True ratings - what they'll become in NFL)
    true_overall_rating INTEGER NOT NULL CHECK(true_overall_rating >= 54 AND true_overall_rating <= 99),
    true_position_ratings TEXT NOT NULL, -- JSON

    -- Scouted Ratings (What scouts think - may be inaccurate)
    scouted_overall_rating INTEGER CHECK(scouted_overall_rating >= 54 AND scouted_overall_rating <= 99),
    scouted_position_ratings TEXT, -- JSON
    ...
    -- Combine Stats
    forty_yard_dash REAL, -- seconds
    bench_press INTEGER, -- reps at 225 lbs
    vertical_jump REAL, -- inches
    broad_jump REAL, -- inches
    three_cone_drill REAL, -- seconds
    twenty_yard_shuttle REAL, -- seconds
    ...
    -- Scouting
    scout_notes TEXT,
    nfl_comparison TEXT, -- "Poor man's [NFL Player]"
    injury_flags TEXT, -- JSON array of injury concerns
...
CREATE INDEX idx_prospects_year ON draft_prospects(draft_year);
CREATE INDEX idx_prospects_position ON draft_prospects(position);
CREATE INDEX idx_prospects_grade ON draft_prospects(draft_grade);
CREATE INDEX idx_prospects_drafted ON draft_prospects(is_drafted);
CREATE INDEX idx_prospects_rating ON draft_prospects(true_overall_rating);

-- ============================================================================
-- 16. Team Draft Boards Table (Phase 1 - Draft System)
-- ============================================================================
CREATE TABLE team_draft_boards (
    board_id INTEGER PRIMARY KEY AUTOINCREMENT,

    team_id INTEGER NOT NULL,
    prospect_id INTEGER NOT NULL,
    rank INTEGER NOT NULL, -- Team's ranking of this prospect

    -- Team-specific notes
    team_grade TEXT, -- Team's own grade
    team_notes TEXT,
...
CREATE INDEX idx_draft_board_team ON team_draft_boards(team_id);
CREATE INDEX idx_draft_board_rank ON team_draft_boards(team_id, rank);

-- ============================================================================
-- 17. Draft Selections Table (Phase 1 - Draft System)
-- ============================================================================
CREATE TABLE draft_selections (
    ...
    -- Pick metadata
    time_on_clock INTEGER DEFAULT 300, -- Seconds remaining when pick made (5 min = 300s)
...
CREATE INDEX idx_draft_selections_year ON draft_selections(draft_year);
CREATE INDEX idx_draft_selections_team ON draft_selections(team_id);
CREATE INDEX idx_draft_selections_overall ON draft_selections(overall_pick);

-- ============================================================================
-- 18. Draft Pick Trades Table (Phase 1 - Draft System)
-- ============================================================================
CREATE TABLE draft_pick_trades (
    ...
    -- Picks/Players from Team 1
    team1_picks TEXT NOT NULL, -- JSON array of pick_ids
    team1_players TEXT, -- JSON array of player_ids (if any)

    -- Picks/Players from Team 2
    team2_picks TEXT NOT NULL, -- JSON array of pick_ids
    team2_players TEXT, -- JSON array of player_ids (if any)

    -- Trade evaluation
    team1_value INTEGER, -- Trade chart value
    team2_value INTEGER,

    -- Status
    status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending', 'accepted', 'rejected', 'countered')),
    proposed_by_team INTEGER NOT NULL, -- 1 or 2
...
CREATE INDEX idx_draft_trades_teams ON draft_pick_trades(team1_id, team2_id);
CREATE INDEX idx_draft_trades_status ON draft_pick_trades(status);
